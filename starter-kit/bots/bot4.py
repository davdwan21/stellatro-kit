...
# ---------------------------------------------------------------------------
# Soft score floor — 40% of fixed_synergy's base values (raised from 20%).
# Data showed 20% was too weak; bot3 was losing marginal 2k-5k games to
# fixed_synergy's stronger priors in seeds where our synergy table had no hits.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Pair-synergy table — scaled up 50% from original conservative values.
# The 13 known pairs were correct but underweighted; fixed_synergy's full
# values were outcompeting us in marginal drafts on seeds with these combos.
# ---------------------------------------------------------------------------
...
def _joker_floor_by_name(name: str) -> float:
    """Floor lookup by name string — used for opponent denial pricing."""
    return _JOKER_FLOOR.get(name, 0.0)
...
class Bot:
    ...
    def pick_joker(self, state: GameState) -> int:
        ...
        for index, joker_model in enumerate(state.joker_pool):
            ...
            # Denial bonus for blocking a high-floor joker from the opponent.
            # Weighted by denial_weight so it scales with seat/pick position.
            opp_floor_deny = _denial_weight(pick_num, is_p1, joker_name) * _joker_floor_by_name(joker_name)

            val = my_score + syn + engine + deny + opp_engine + floor + opp_floor_deny
        ...
        if my_turn:
            ...
            for index, joker_cls in enumerate(pool):
                ...
                val = (
                    ...
                    + _denial_weight(current_pick, is_p1, joker_name) * (opp_gain + _joker_floor_by_name(joker_name))
