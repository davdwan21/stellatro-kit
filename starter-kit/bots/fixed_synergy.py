...
def _get_diverse_candidates(
    ...
    for combo in combinations(range(n), 5):
        ...
        shape = (
            7 * sum(ranks)
            + 65 * (len(ranks) - len(set(ranks)))
            + (220 if len(set(suits)) == 1 else 0)
            + 40 * sum(1 for r in ranks if r in {11, 12, 13})
        )
...
class Bot:
    ...
    def pick_joker(self, state: GameState) -> int:
        ...
        for index, joker_cls in enumerate(pool):
            ...
            val = my_score + syn + combo + deny + opp_combo_deny
        ...
        try:
            ...
            for target_depth, beam_k in ((1, 7), (2, 3)):
                self._timeout()
